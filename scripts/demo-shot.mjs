// Скриншот таблицы замечаний в демо-проекте. Помогает глазами проверить локальный стенд.
...
for (let i = 0; i < (await rows.count()); i += 1) {
  console.log(" —", (await rows.nth(i).innerText()).replace(/\s+/g, " ").slice(0, 80));
}
...
const stage = page.getByRole("button", { name: /^Замечания (\d+\/\d+|—)$/ });
console.log("этап «Замечания»:", await stage.count(), await stage.first().innerText().catch(() => "—"));
