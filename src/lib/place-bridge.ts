...
/**
 * Показывает место в рабочей вкладке. `false` — вкладки нет, вызывающий сам
 * решает, что делать (открыть место у себя).
 *
 * Сам фокус браузер по `focus()` переводит не всегда (Chrome не даёт вкладке
 * перетягивать внимание), поэтому возврат довершает вызывающий: если фокус
 * остался здесь, эту вкладку закрывают — браузер вернёт на родительскую.
 */
