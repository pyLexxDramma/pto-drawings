...
/**
 * Служебные разделы конвейера. Ищем по образцу: формулировки он меняет — на
 * проде это «Геометрия листа (из векторов PDF, точно)» и «Информация о листе»
 * с size_pt/text_len/reasons. «Описание чертежа» инженер читает первым, его
 * не сворачиваем. Дословный текст листа тоже остаётся раскрытым.
 */
const SERVICE_SECTION =
  /(состав\s+лист|геометри|карта\s+лист|информаци\S*\s+о\s+лист|якор)/i;
